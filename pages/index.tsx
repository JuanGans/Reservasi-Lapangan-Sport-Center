  ...
  return (
    <div className="scroll-smooth font-[Poppins]">
      ...
      <section id="home" className="relative min-h-screen flex flex-col items-center justify-center bg-cover bg-center text-center px-4" style={{ backgroundImage: "url('/futsal.jpg')" }}>
        <nav className="absolute top-0 left-0 right-0 z-50 py-4 px-6 flex justify-between items-center">
          ...
          <div className="flex flex-1 justify-center">
            <ul className="flex space-x-10 text-white font-medium text-lg">
              ...
              <li>
                <a href="#feature" className="hover:text-blue-300">
                  Feature
                </a>
              </li>
              <li>
        ...
        <h1 className="text-white text-4xl md:text-6xl font-bold mb-4 drop-shadow-lg mt-24">Selamat Datang di Website Sport Center JTI POLINEMA!</h1>
        <p className="text-white max-w-2xl drop-shadow-md">Tempat booking lapangan, jadwal latihan, dan informasi seputar sport center JTI Polinema dengan mudah dan cepat.</p>
      </section>

      {/* About */}
      <section id="about" className="bg-blue-100 py-10">
        <div className="text-center text-blue-900 text-2xl font-semibold">Tentang</div>
        <p className="text-blue-900 max-w-xl mx-auto drop-shadow-md">Platform digital untuk melakukan pemesanan lapangan dan melihat jadwal kegiatan olahraga di lingkungan JTI Polinema.</p>
      </section>
      ...
      <section id="feature" className="bg-white text-center py-16 mb-16">
        ...
        <p className="text-gray-600 mb-6 max-w-xl mx-auto">Dengan proses pendaftaran yang simpel dan sistem booking lokal, reservasi jadwalmu makin praktis.</p>
      ...
      <section className="bg-blue-100 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 px-6 py-10 mb-16">
        {fields.map((field, idx) => (
          <div key={idx} className="bg-white shadow-md rounded-lg p-4 hover:scale-105 transition cursor-pointer" onClick={() => setSelectedField(field)}>
      ...
      {selectedField && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70">
          <div className="bg-white p-6 rounded-lg max-w-md w-full text-center relative">
            <button className="absolute top-2 right-3 text-gray-600 hover:text-black text-xl" onClick={() => setSelectedField(null)}>
              ×
            </button>
      ...
      <section id="team" className="relative text-center mb-16 py-16 px-4 overflow-hidden">
        <div className="absolute inset-0 bg-cover bg-center opacity-30 blur-sm" style={{ backgroundImage: "url('https://c4.wallpaperflare.com/wallpaper/483/576/977/abstract-hexagon-simple-minimalism-wallpaper-preview.jpg')" }}></div>
