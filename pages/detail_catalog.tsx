  ...
  return (
    <>
      ...
      <MainLayout title="Detail Lapangan">
        {/* HERO SECTION */}
        {field ? (
          <section className="relative min-h-[300px] flex flex-col items-center justify-center bg-cover bg-center text-center text-white px-4" style={{ backgroundImage: `url(${field.field_image || "/assets/field/fallback.jpg"})` }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white p-8 rounded-xl shadow">
              <div>
                <LandingAnimation delay={0.2}>
                  <img src={field.field_image || "/assets/field/fallback.jpg"} alt={field.field_name} className="w-full h-[360px] object-cover rounded-lg" />
                </LandingAnimation>
                <LandingAnimation>
            ...
            {recommended.length > 0
              ? recommended.map((field, idx) => (
                  <LandingAnimation key={idx} delay={idx * 0.1}>
                    <div key={field.id} onClick={() => router.push(`/detail_catalog?id=${field.id}`)} className="cursor-pointer bg-white rounded-lg shadow hover:shadow-lg transition p-4">
                      <img src={field.field_image || "/assets/field/fallback.jpg"} alt={field.field_name} className="w-full h-40 object-cover rounded mb-3" />
